...
const SocialIcon = ({ path, href, colorClass, borderClass }: { path: string, href: string, colorClass: string, borderClass: string }) => (
  <a 
    href={href} 
    target="_blank" 
    rel="noopener noreferrer"
    className={`group relative p-3 bg-slate-900/80 rounded-lg border border-white/10 ${borderClass} transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_0_15px_rgba(255,255,255,0.3)]`}
  >
    {/* Icon SVG */}
    <svg 
      viewBox="0 0 24 24" 
      fill="currentColor" 
      className={`w-6 h-6 text-slate-400 ${colorClass} transition-colors`}
    >
      <path d={path} />
    </svg>
  </a>
...
export default function Footer() {
  return (
    // FIXED:
    // 1. z-50: Forces it above any background animations or glitched layers.
    // 2. bg-black/40: Much lighter transparency.
    // 3. backdrop-blur-xl: Stronger glass effect to compensate for lighter background.
    // 4. pb-[...]: Adds specific padding for iPhone Home Bar (Safe Area).
    <footer className="relative z-50 mt-20 border-t border-white/10 bg-black/40 backdrop-blur-xl pb-8 mb-[env(safe-area-inset-bottom)]">
      ...
      <div className="max-w-7xl mx-auto p-8 md:p-12">
        <div className="flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          <div className="text-center md:text-left">
            <h2 className="font-sans font-black text-2xl text-white italic tracking-tighter uppercase mb-2">
              MINI BATTLE <span className="text-mbl-yellow">LEAGUE</span>
            </h2>
            <p className="text-slate-500 font-sans font-bold text-xs uppercase tracking-widest">
          ...
          <div className="flex gap-4">
            ...
            <SocialIcon 
              ...
              colorClass="group-hover:text-mbl-pink"
              borderClass="hover:border-mbl-pink"
